...
# Load intent configuration (this could also come from a DB or static file)
with open("intent_config.json", "r") as f:
    intent_config = json.load(f)

# Load client data from exported JSON file
...
def extract_company(text: str):
    try:
        companies = set([doc["company"] for doc in client_data])
